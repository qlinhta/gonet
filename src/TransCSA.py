...
class CSA(tf.keras.layers.Layer):
    def __init__(self, in_dim, out_dim, num_heads, kernel_size=3, padding=1, stride=2,
        ...
        self.stride = stride
        self.scale = qk_scale or (out_dim // num_heads) ** -0.5

        self.attn = layers.Dense(kernel_size ** 4 * num_heads)
        self.attn_drop = layers.Dropout(attn_drop)

        self.unfold = layers.Conv2D(out_dim, kernel_size, strides=stride, padding='same')
        self.pool = layers.AveragePooling2D(pool_size=stride, strides=stride, padding='same')

        self.csa_group = 1
        assert out_dim % self.csa_group == 0
        self.weight = layers.Conv2D(
            ...
            use_bias=qkv_bias,
            kernel_initializer=tf.keras.initializers.RandomNormal(mean=0.0, stddev=math.sqrt(
                2.0 / (self.kernel_size * self.kernel_size * self.out_dim / self.csa_group)))
        )
    ...
    def call(self, x, v=None):
        B, H, W, _ = tf.shape(x)
        h, w = math.ceil(H / self.stride), math.ceil(W / self.stride)

        attn = self.pool(tf.transpose(x, perm=[0, 3, 1, 2]))
        attn = tf.transpose(attn, perm=[0, 2, 3, 1])
        attn = self.attn(attn)
        attn = tf.reshape(
            attn, (B, h * w, self.num_heads, self.kernel_size * self.kernel_size, self.kernel_size * self.kernel_size)
        )
        attn = tf.transpose(attn, perm=[0, 2, 1, 3, 4])  # B,H,N,kxk,kxk
        attn = attn * self.scale
        attn = tf.nn.softmax(attn, axis=-1)
        attn = self.attn_drop(attn)

        v = tf.transpose(x, perm=[0, 3, 1, 2])  # B,C,H, W
        v = self.unfold(v)
        v = tf.reshape(v, (B, h * w, self.kernel_size * self.kernel_size, self.out_dim))
        v = tf.transpose(v, perm=[0, 1, 3, 2])
        v = tf.reshape(v, (B * h * w, self.kernel_size * self.kernel_size * self.out_dim, 1, 1))
        v = self.weight(v)
        v = tf.reshape(v,
                       (B, h * w, self.kernel_size * self.kernel_size, self.num_heads, self.out_dim // self.num_heads))
        v = tf.transpose(v, perm=[0, 3, 1, 2, 4])  # B,H,N,kxk,C/H

        x = tf.matmul(attn, v)
        x = tf.transpose(x, perm=[0, 1, 4, 3, 2])
        x = tf.reshape(x, (B, self.out_dim * self.kernel_size * self.kernel_size, h * w))
        x = tf.raw_ops.Fold(input=x, output_shape=[H, W], block_size=self.kernel_size,
                            paddings=[[self.padding, self.padding], [self.padding, self.padding]],
...
x = _resnet(input)
csa_layer = CSA(in_dim=res_filters, out_dim=res_filters, num_heads=num_heads, kernel_size=3, padding=1, stride=2)
x = csa_layer(x)
